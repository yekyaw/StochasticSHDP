...
class suff_stats:
    ...
    def set_zero(self):
        ...
        self.m_dmu_ss.fill(0.0)

class online_hdp:
    ''' hdp model using stick breaking'''
    def __init__(self, C, T, K, D, W, eta, alpha, gamma, kappa, tau, scale=1.0, adding_noise=False,
                 penalty_lambda=1., l1_ratio=0.6):
        """
    ...
    #     ss.m_dmu_ss = [dmu + self._deriv_mu(mu, C, Epi_dot_c, y) \
    ...
    def doc_e_step_infer(self, doc, Elogsticks_1st, var_converge, max_iter=100):
        ...
        # back to the uniform
        phi = np.ones((len(doc.words), self.m_K)) * 1.0 / self.m_K
        var_phi = np.ones((self.m_K, self.m_T)) * 1.0 / self.m_T
        Epi_dot_c = Esticks_2nd.dot(var_phi)
    ...
    def infer_only(self, docs, var_converge):
        ...
        Epi_dot_cs = np.zeros((len(docs), self.m_T))
        Elogsticks_1st = expect_log_sticks(self.m_var_sticks)
        for i, doc in enumerate(docs):
            (doc_score, Epi_dot_c) = self.doc_e_step_infer(doc, Elogsticks_1st, var_converge)
            likelihood += doc_score
            Epi_dot_cs[i] = Epi_dot_c
            preds[i,:] = self.predict(Epi_dot_c)
        return (likelihood, preds, Epi_dot_cs)
