...
class Poisson(GLM):
    ...
    def likelihood(self, omega, counts, N, y):
        ...
        likelihood -= self._expected_log_norm(omega, counts, N)
        return likelihood
...
class Categorical(GLM):
    ...
    def _expected_exps_parts(self, omega, counts, N):
        mu_exp = np.exp(self.mu / N)
        parts = mu_exp.dot(omega.T)
        parts = np.apply_along_axis(lambda x: np.power(x, counts), 1, parts)
        return parts
    def _expected_log_norm(self, omega, counts, N):
        parts = self._expected_exps_parts(omega, counts, N)
    ...
    def dphi(self, phi, n, var_phi, counts, N, y, xnorm=None):
        ...
        exps_parts = self._expected_exps_parts(phi.dot(var_phi), counts, N)
        exps = np.prod(exps_parts, 1)
        denom = np.sum(exps)
        for c in range(self.C):
            exp_parts_minus_n = exps[c] / (exps_parts[c,n] + 1e-100)
            mu_exp = np.exp(self.mu[c,:] * counts[n] / N)
            coef = exp_parts_minus_n * var_phi.dot(mu_exp) / (denom + 1e-100)
            dphi -= deriv_helper(xnorm, coef)
        return dphi
    def dvar_phi(self, var_phi, i, phi, counts, N, y, xnorm=None):
        phi_mean = phi[:,i].dot(counts) / N
    ...
    def dmu(self, omega, counts, N, y):
        ...
        eta = compute_eta(omega, counts, N)
        dmu[y,:] += eta
        exps_parts = self._expected_exps_parts(omega, counts, N)
        exps = np.prod(exps_parts, 1)
        denom = np.sum(exps)
        for c in range(self.C):
            exp_parts_minus_n = exps[c] / (exps_parts[c,:] + 1e-100)
            term = (exp_parts_minus_n * counts).dot(omega) / N
            dmu[c,:] -= term * np.exp(self.mu[c,:] / N) / (denom + 1e-100)
        dmu[np.isnan(dmu)] = 0.
        return dmu
    def requires_fp(self):
        return True
...
class Bernoulli(GLM):
    ...
    def predict(self, omega, counts, N):
        ...
        if mean > 0:
            ...
            return 0
    def _expected_exp_parts(self, omega, counts, N):
        mu_exp = np.exp(self.mu / N)
        return np.power(omega.dot(mu_exp), counts)
    def _expected_log_norm(self, omega, counts, N):
        parts = self._expected_exp_parts(omega, counts, N)
    ...
    def dphi(self, phi, n, var_phi, counts, N, y, xnorm=None):
        dphi = deriv_helper(xnorm, y * var_phi.dot(self.mu) * counts[n] / N)
        exp_parts = self._expected_exp_parts(phi.dot(var_phi), counts, N)
    ...
    def dmu(self, omega, counts, N, y):
        eta = compute_eta(omega, counts, N)
        dmu = y * eta
        exp_parts = self._expected_exp_parts(omega, counts, N)
        exp_prod = np.prod(exp_parts)
        denom = 1 + exp_prod
        exp_parts_minus_n = exp_prod / (exp_parts + 1e-100)
        term = (exp_parts_minus_n * counts).dot(omega) / N
        dmu -= term * np.exp(self.mu / N) / (denom + 1e-100)
        dmu[np.isnan(dmu)] = 0.
        return dmu
    def requires_fp(self):
        return True    
